...
def checkHGT(hgt):
    cm = re.match('^([0-9]+)cm', hgt)
    inch = re.match(r'^([0-9]+)in', hgt)
    if cm:
        if int(cm[1]) in range (150,194):
            return 1
        else:
            return 0
    elif inch:
        if int(inch[1]) in range (59,77):
            return 1
        else:
            return 0
    else:
